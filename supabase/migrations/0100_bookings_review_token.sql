-- Surface the review token already generated at completion (0099) through
-- bookings_by_email, so the customer dashboard can offer "Leave a review"
-- for completed sessions without a second token-generation path — if they
-- lose the email, the same link/token still works via the dashboard button.
...
returns table (
  ...
  review_token uuid, review_status text, review_rating int
  ...
  left join review_email_tokens ret on ret.booking_id = b.id
