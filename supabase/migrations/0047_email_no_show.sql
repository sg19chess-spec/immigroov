-- notify_booking_event gains the 'no_show' event (role-specific by who missed it).
create or replace function notify_booking_event(p_booking_id bigint, p_event text)
returns void language plpgsql security definer set search_path = public as $$
declare
  d record; o record;
  s_title text; v_meeting text; v_dur int; v_amount numeric; v_cur text; v_no_show_by text;
...
begin
  select coalesce(b.guest_email, cu.email), coalesce(nullif(cu.first_name,''),'there'),
         mu.email, coalesce(nullif(mu.first_name,''),'there'), s.title, b.meeting_url, s.duration, b.no_show_by
    into mentee_email, mentee_name, mentor_email, mentor_name, s_title, v_meeting, v_dur, v_no_show_by
  from bookings b
  ...
  if p_event in ('confirmed','rescheduled') and v_meeting is not null then
