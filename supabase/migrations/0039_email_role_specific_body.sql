-- Each recipient (mentee / mentor / admin) gets body text written for THEIR role
-- (previously the greeting name was swapped but the body was shared, so e.g. the
-- mentor's copy said "pick a slot that works for you").
...
begin
  ...
  if p_event = 'confirmed' then
    ...
    ld_m := 'Your session is confirmed — all the details are below.';
    ld_r := 'Your session with '||mentee_name||' is confirmed.';
    ld_a := 'A session was confirmed.';
  elsif p_event = 'cancelled' then
    heading := 'Your session was cancelled';
    ld_m := 'Your session has been cancelled. Details below for your records.';
    ld_r := 'Your session with '||mentee_name||' was cancelled.';
    ld_a := 'A session was cancelled.';
  elsif p_event = 'rescheduled' then
    heading := 'Your session was rescheduled';
    ld_m := 'Your session has been moved to a new time — updated details below.';
    ld_r := 'Your session with '||mentee_name||' has been moved to a new time.';
    ld_a := 'A session was rescheduled.';
  elsif p_event = 'completed' then
    heading := 'Your session is complete';
    ld_m := 'Thanks for using Immigroov! We''d love your feedback on this session.';
    ld_r := 'Your session with '||mentee_name||' is complete.';
    ld_a := 'A session was completed.';
  elsif p_event = 'proposed' then
    heading := 'A new time was proposed';
    ld_m := 'Your mentor proposed a new time window — your current time hasn''t changed yet. Pick a slot that works for you.';
    ld_r := 'You proposed a new time window. We''ll let '||mentee_name||' pick a slot inside it.';
    ld_a := mentor_name||' proposed a new time window for '||mentee_name||'.';
    ...
    heading := 'A new time was selected';
    ld_m := 'You picked a new time — waiting for your mentor to confirm it.';
    ld_r := mentee_name||' picked a time inside your proposed window. Confirm it to lock in the reschedule.';
    ld_a := mentee_name||' selected a time; awaiting mentor confirmation.';
    ...
    if o.selected_time is not null then
      extra := '<div style="background:#fff2e8;border:1px solid #fb7321;border-radius:14px;padding:14px;margin-top:12px;font-size:13px;color:#0c1b33;"><b>Selected time:</b><br>'
    ...
    ld_m := 'You asked for a different day — waiting for your mentor to propose times for it.';
    ld_r := mentee_name||' can''t make the proposed day and asked for another date. Please propose a time range for it.';
    ld_a := mentee_name||' requested a different day.';
  ...
  if to_admin is not null then
    msgs := msgs || jsonb_build_object('to', to_admin, 'subject', replace(tag,'%role%','ADMIN')||'Immigroov ['||p_event||']: '||s_title,
      'html', email_layout(heading, '<p style="font-size:14px;color:#0c1b33;margin:0;">'||ld_a||' &middot; booking #'||p_booking_id||'</p>'||det||extra, 'Open console', dash_url));
  end if;
