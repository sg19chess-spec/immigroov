-- Send all recipients for one event in a single Resend request (avoids the
-- per-email rate limit that drops messages when 3 fire at once on the free tier).
...
begin
  ...
  select jsonb_agg(jsonb_build_object('from', v_from, 'to', m->>'to', 'subject', m->>'subject', 'html', m->>'html'))
    into v_body
