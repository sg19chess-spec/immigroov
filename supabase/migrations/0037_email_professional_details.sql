-- Professional booking emails: full details table (service, duration, both
-- parties, both timezones with full dates, video link, amount paid, booking ref).
create or replace function email_row(p_label text, p_val text) returns text language sql immutable as $$
  select '<tr><td style="padding:9px 14px;color:#6b7689;border-top:1px solid #eef1f7;width:42%;vertical-align:top;">'||p_label||'</td>'
       ||'<td style="padding:9px 14px;color:#0c1b33;font-weight:600;border-top:1px solid #eef1f7;vertical-align:top;">'||coalesce(nullif(p_val,''),'—')||'</td></tr>';
$$;
...
begin
  ...
  det := '<div style="border:1px solid #e8ecf4;border-radius:14px;overflow:hidden;margin-top:16px;">'
       ...
       ||email_row('Your time (mentee)', when_mentee)
       ||email_row('Mentor''s time', when_mentor)
