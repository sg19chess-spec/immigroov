-- Fix: process_referral_commissions() stored commission_amount_inr straight
-- from customer_payments.amount with no FX conversion. For any customer who
-- paid in a non-INR currency, the ledger recorded (e.g.) their USD amount
-- but labeled/summed/displayed it everywhere downstream as INR.
--
-- add_ledger() (0063_money_model.sql) already computes the correct converted
-- figure for this exact same commission — booking_ledger.normalized_inr_amount
-- — using bookings.fx_customer_inr. That conversion is applied here too, so
-- commission_amount_inr is genuinely INR-normalized as its name promises.
-- Nothing else changes: fraud thresholds, payout batches, and both dashboards
-- all read this one column, so fixing it here fixes them transitively.
