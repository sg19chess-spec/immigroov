-- Real bug found while verifying the admin UI: the function's own RETURNS
-- TABLE column "affiliate_id" shadowed the plain affiliate_id column
-- references inside the subqueries (commission_ledger/fraud_flags both have
-- a column of that same name), so Postgres couldn't tell which one was meant
-- and the whole function errored — silently rendered as "0 affiliates" in the
-- UI since the frontend didn't surface the RPC error either. Fixed by
-- qualifying every reference explicitly.
