-- test_redirect_email modes:
--   ''               -> live (real mentor / mentee / admin)
--   'mentee'         -> all 3 copies go to THIS booking's mentee (logged-in) email
--   'someone@x.com'  -> all 3 copies go to that fixed inbox
...
begin
  ...
  if redirect = 'mentee' then redirect := mentee_email; end if;   -- route to the logged-in mentee
...
-- switch test mode: all 3 copies go to the logged-in mentee's own email
select demo_set_setting('test_redirect_email', 'mentee');
