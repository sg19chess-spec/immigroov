-- Two-button email template + per-event/role CTAs that deep-link to the exact
-- page (mentor links land on the Sessions tab; Join uses the meeting link).
-- Drop the earlier 4-arg signature so the new optional params don't create an
-- ambiguous overload.
drop function if exists email_layout(text, text, text, text);
create or replace function email_layout(
  p_heading text, p_body text,
  p_cta_label text default null, p_cta_url text default null,
  p_cta2_label text default null, p_cta2_url text default null)
returns text language sql immutable as $$
 ...
 ||case when p_cta_url is not null then
     ...
     ||'<a href="'||p_cta_url||'" style="display:inline-block;background:linear-gradient(135deg,#fb7321,#fa5a2b);color:#fff;text-decoration:none;font-weight:700;font-size:14px;padding:12px 20px;border-radius:12px;">'||coalesce(p_cta_label,'View')||'</a>'
     ||case when p_cta2_url is not null then
         '&nbsp;&nbsp;<a href="'||p_cta2_url||'" style="display:inline-block;background:#fff;color:#0a2240;text-decoration:none;font-weight:700;font-size:14px;padding:11px 18px;border-radius:12px;border:1px solid #d8dEEa;">'||coalesce(p_cta2_label,'More')||'</a>'
        else '' end
     ||'</div>'
    else '' end
...
declare
  ...
  ml1 text; mu1 text; ml2 text; mu2 text;   -- mentee buttons
  rl1 text; ru1 text; rl2 text; ru2 text;   -- mentor buttons
begin
