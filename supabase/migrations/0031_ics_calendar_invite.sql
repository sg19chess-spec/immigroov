-- Attach an .ics calendar invite to confirmed / rescheduled / cancelled emails.

create or replace function ical_esc(t text) returns text language sql immutable as $$
  select replace(replace(replace(replace(coalesce(t,''),'\','\\'),';','\;'),',','\,'), E'\n', '\n');
$$;

-- Build an iCalendar (VEVENT) for a booking. Same UID across the lifecycle so
-- calendars update the existing event; STATUS flips to CANCELLED on cancel.
create or replace function booking_ics(p_booking_id bigint, p_cancelled boolean default false)
returns text language plpgsql stable security definer set search_path = public as $$
declare b record; v_end timestamptz; crlf text := chr(13)||chr(10); ics text;
begin
  select bk.slot_time, bk.meeting_url, s.title, coalesce(s.duration,30) as duration,
  ...
  ics := 'BEGIN:VCALENDAR'||crlf
    ...
    ||coalesce('LOCATION:'||ical_esc(b.meeting_url)||crlf,'')
    ||'DESCRIPTION:'||ical_esc('Session with '||b.mentor_name||case when b.meeting_url is not null then E'\nJoin: '||b.meeting_url else '' end)||crlf
    ||'END:VEVENT'||crlf
    ||'END:VCALENDAR'||crlf;
  return ics;
end; $$;

-- Batch sender now passes through per-message attachments
create or replace function app_send_email_batch(p_msgs jsonb)
returns bigint language plpgsql security definer set search_path = public as $$
declare v_key text; v_from text; v_req bigint; v_body jsonb;
begin
  select decrypted_secret into v_key from vault.decrypted_secrets where name = 'resend_api_key';
  if v_key is null or p_msgs is null or jsonb_array_length(p_msgs) = 0 then
    raise notice 'app_send_email_batch: no key or no messages — skipping'; return null;
  end if;
  ...
  select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
           'from', v_from, 'to', m->>'to', 'subject', m->>'subject', 'html', m->>'html',
...
-- notify now attaches the .ics for confirmed / rescheduled / cancelled
...
begin
  ...
  if to_mentee is not null then
    msgs := msgs || (jsonb_build_object('to', to_mentee, 'subject', replace(tag,'%role%','MENTEE')||'Immigroov: '||heading,
      'html', email_layout(heading, '<p style="font-size:14px;color:#0c1b33;">Hi '||mentee_name||',</p><p style="font-size:14px;color:#0c1b33;">'||lead||'</p>'||card, 'View your session', site||'/bookings'))
  ...
  if to_mentor is not null then
    msgs := msgs || (jsonb_build_object('to', to_mentor, 'subject', replace(tag,'%role%','MENTOR')||'Immigroov: '||heading,
      'html', email_layout(heading, '<p style="font-size:14px;color:#0c1b33;">Hi '||mentor_name||',</p><p style="font-size:14px;color:#0c1b33;">'||lead||'</p>'||card, 'Open mentor console', site||'/dashboard'))
      || (case when att is not null then jsonb_build_object('attachments', att) else '{}'::jsonb end));
  end if;
  if to_admin is not null then
    msgs := msgs || jsonb_build_object('to', to_admin, 'subject', replace(tag,'%role%','ADMIN')||'Immigroov ['||p_event||']: '||s_title,
      'html', email_layout(heading, '<p style="font-size:14px;color:#0c1b33;">Admin notification.</p><p style="font-size:14px;color:#0c1b33;">'||lead||'</p>'||card, 'Open console', site||'/dashboard'));
  end if;
