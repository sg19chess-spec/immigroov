-- =============================================================================
-- 0071 — Consolidate the booking lifecycle family to ONE correct definition.
--
-- Corrects regressions from 0066/0067/0070, which rewrote several lifecycle
-- functions from PRE-0051 bodies and thereby (a) dropped the log_event() audit
-- calls 0051 added, and (b) re-created the 2-arg respond_booking_request that
-- 0051 had deliberately dropped — leaving two anon overloads (PostgREST
-- ambiguity risk on mentor request-approval).
--
-- This migration re-creates each function from the 0051 CANONICAL body and then
-- layers the good changes on top:
--   • FOR UPDATE on every gating row read (concurrency / double-write guard, 0070)
--   • resolve_mentor_no_show closes its status window on all branches (0067 fix)
--   • single respond_booking_request(bigint,boolean,text) — the 2-arg is dropped
--   • resolve_expired_requests calls it with 'system' + FOR UPDATE SKIP LOCKED
--   • admin_booking_detail prefers booking_events (0051) MERGED with the
--     fee_pct/platform_take/net_to_mentor/mentor_currency/ppp_multiplier fields
--     from 0066.
-- Money math is unchanged throughout.
-- =============================================================================

-- Remove the duplicate 2-arg overload resurrected by 0070 (0051 had dropped it).
...
-- ---------- admin detail: 0051 events-preferring timeline + 0066 money fields --
